# Důkazní balíček: `PalletRelease`

## FAKTA

- Objekt: `PalletRelease`
- Druh: `SW.Blocks.GlobalDB`
- Číslo: `9001`
- Jazyk: `DB`
- Deklarace (export): `EXPORT/Program blocks/OP010/03_Auto/PalletRelease.xml`
- Riziko: `RIZIKOVÉ`
...
- Nálezy `CallInfo Name="PalletRelease"`: `0`
- Nálezy `Text="PalletRelease"` (tokenizovaný StructuredText): `0`
- Nálezy `Component Name="PalletRelease"`: `0`

### Volání bloků (z `deep_reader/out/call_edges.csv`)

- Příchozí volání (kdo volá `PalletRelease`): `0`
- Odchozí volání (co `PalletRelease` volá): `0`
...
- Root `PalletRelease` nebyl nalezen v symbolickém použití (0 záznamů).

### Absolutní adresování DB (orientační scan)

- Pro DB číslo `9001` nebyl nalezen výskyt `DB9001` / `DB 9001` v exportovaných OB/FB/FC (není to důkaz nepoužití).

### Vazby HMI (z `deep_reader/out/hmi_tags.csv`)

- HMI tagy s kořenem `ControllerTag` = `PalletRelease`: `0`

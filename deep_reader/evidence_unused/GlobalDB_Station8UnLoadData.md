# Důkazní balíček: `Station8UnLoadData`

## FAKTA

- Objekt: `Station8UnLoadData`
- Druh: `SW.Blocks.GlobalDB`
- Číslo: `525`
- Jazyk: `DB`
- Deklarace (export): `EXPORT/Program blocks/OP010/11_MES/Station8UnLoadData.xml`
...
- Nálezy `CallInfo Name="Station8UnLoadData"`: `0`
- Nálezy `Text="Station8UnLoadData"` (tokenizovaný StructuredText): `0`
- Nálezy `Component Name="Station8UnLoadData"`: `0`

### Volání bloků (z `deep_reader/out/call_edges.csv`)

- Příchozí volání (kdo volá `Station8UnLoadData`): `0`
- Odchozí volání (co `Station8UnLoadData` volá): `0`
...
- Root `Station8UnLoadData` nebyl nalezen v symbolickém použití (0 záznamů).

### Absolutní adresování DB (orientační scan)

- Pro DB číslo `525` nebyl nalezen výskyt `DB525` / `DB 525` v exportovaných OB/FB/FC (není to důkaz nepoužití).

### Vazby HMI (z `deep_reader/out/hmi_tags.csv`)

- HMI tagy s kořenem `ControllerTag` = `Station8UnLoadData`: `0`
